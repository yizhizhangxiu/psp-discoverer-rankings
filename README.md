...
本仓库将 **PSP 系统发现目标列表** 的网页快照转换为可审计的发现者统计数据，并生成可直接浏览的静态排行页面。项目保留原始数据、结构化处理结果、生成脚本与验证脚本，以便每次更新都能追溯到对应来源。[1]

> **维护频率：列表信息每隔一个月（即每两个月）人工核对并手动更新一次。**

仓库不使用自动抓取或自动发布机制。每次维护均应保存新的源页面快照、重新生成 JSON 和发布页面、执行本地验证，并将相关变更一并提交。
